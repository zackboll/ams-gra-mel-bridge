...
package body AMS_MEL_IR_Tests is
   ...
   begin
      if Value.Pixel_Count /= 12
        or else Value.Width /= 4
        or else Value.Height /= 3
        or else Value.Bits_Per_Pixel /= 8
        or else Value.Number_Of_Bands /= 1
        or else Value.Frame_ID /= Interfaces.Unsigned_32 (ID)
        or else Value.Subframe_ID /= 2
        or else Value.Subframe_Total /= 4
        or else Value.Kind /= AMS.MEL.IR.Staring
        or else Value.Flip /= AMS.MEL.IR.Horizontal
        or else Value.Image_Flags /= 4
        or else Value.Row_Offset /= 7
        or else Value.Column_Offset /= 9
        or else Value.Band_Index /= 3
        or else Value.Horizontal_FOV_Rad /= 0.25
        or else Value.Vertical_FOV_Rad /= 0.125
        or else Value.Dither_Row /= 0.5
        or else Value.Dither_Column /= -0.25
      then
   ...
   begin
      ...
      begin
         Check_Frame (First, 1);
         Check_Frame (Second, 2);
         if Counts.Frames_Received < 2 or else Counts.Malformed_Or_Unsupported /= 0 then
   ...
   begin
      ...
      begin
         ...
         begin
            raise Program_Error
              with "failed provider returned frame" & Unexpected.Pixel_Count'Image;
         end;
      exception
