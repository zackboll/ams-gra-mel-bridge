...
package body AMS_MEL_IR_Health_Status_Tests is
   ...
   Config : constant H.Health_Config :=
     H.Create_Config
   ...
   begin
      H.Enable (Channel);
      declare
         Capability : constant V.Channel_Capability := H.Capabilities (Channel);
      begin
         ...
         then
            raise Program_Error with "Health capability mismatch";
         end if;
      end;
      declare
      ...
      begin
         ...
         then
            raise Program_Error with "rich MFA_Status mismatch";
         end if;
      end;
      declare
         Event     : constant M.Metadata_Event := M.Receive (Stream, 1_000);
      ...
      begin
         ...
         then
            raise Program_Error with "rich BIT_Status mismatch";
         end if;
      end;
      declare
         Event : constant M.Metadata_Event := M.Receive (Stream, 1_000);
      ...
      begin
         ...
         then
            raise Program_Error with "rich SubsystemStatusResp mismatch";
         end if;
      end;
      declare
         Event : constant M.Metadata_Event := M.Receive (Stream, 1_000);
      ...
      begin
         ...
         then
            raise Program_Error with "rich DiscreteStatus mismatch";
         end if;
      end;
      for Expected in S.Security_Event_Kind loop
         ...
         begin
            if S.Kind (Detail) /= Expected
              or else S.Timestamp_NS (Value) /= -987_654_321
              or else S.Artifact_Count (Value) /= 2
              or else S.Outcome (Value) /= S.Failure
              or else S.Severity (Value) /= S.Warning
              or else (Expected = S.Authentication
                       and then (S.Category (Detail) /= 5
                                 or else S.Details (Detail) /= "auth-ζ"
                                 or else AMS.MEL.IR.Descriptive_Label (S.Service_ID (Detail))
                                         /= "auth service"))
            then
               raise Program_Error with "rich SecurityAudit variant mismatch";
            end if;
         end;
      end loop;
      declare
      ...
      begin
         ...
         then
            raise Program_Error with "Ada-owned MFA_StatusDetailed mismatch";
         end if;
      end;
      begin
         declare
   ...
   begin
      if Counts.Events_Received /= 12
        or else Counts.Events_Dropped_Queue_Full /= 10
        or else Counts.Malformed_Or_Unsupported /= 0
      then
         raise Program_Error with "Health DROP-INCOMING counters mismatch";
      end if;
      if M.Kind (M.Receive (Stream)) /= M.MFA_Status_Event
        or else M.Kind (M.Receive (Stream)) /= M.BIT_Status_Event
      then
         raise Program_Error with "Health DROP-INCOMING order mismatch";
      end if;
      M.Close (Stream);
      H.Enable (Channel);
      H.Close (Channel);
