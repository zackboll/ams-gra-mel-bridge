...
package body AMS_MEL_IR_C2_Common_Tests is
   ...
   package Common renames C2.Common;
   package M renames C2.Metadata;
   ...
   begin
      ...
      declare
         Keepalive : C2.Return_Request := Common.Send_Keep_Alive (Control);
      begin
         if C2.Value (C2.Wait (Keepalive, 1_000)) /= C2.Return_Success
           or else C2.Value (C2.Wait (Keepalive, 0)) /= C2.Return_Success
         then
      ...
      declare
         Request  : Common.Comms_Request :=
           Common.Submit_Comms_Test (Control, 16#F000_0002#, 16#8000_0001#, 16#E000_0003#);
      ...
      begin
         if M.Kind (Event) /= M.Channel_Comms_Test_Event
           or else Callback.Command_ID /= 16#8000_0001#
           or else Callback.Request_ID /= 16#E000_0003#
           or else Common.Status (Result) /= C2.Success
           or else Report.Command_ID /= Callback.Command_ID
           or else Report.Request_ID /= Callback.Request_ID
           or else Common.Report (Common.Wait (Request, 0)) /= Report
         then
            raise Program_Error with "Ada CommsTest mismatch";
         end if;
   ...
   begin
      ...
      then
         raise Program_Error with "Ada ChannelCapability mismatch";
      end if;
   end Test_Capability;
   ...
   begin
      declare
         Parent  : AMS.MEL.Session := AMS.MEL.Open (Provider_Path, "comms-delayed");
         Control : C2.Control_Channel := C2.Open (Parent, Config);
         Request : Common.Comms_Request := Common.Submit_Comms_Test (Control, 7, 17, 19);
      begin
         begin
            declare
               Unexpected : constant Common.Comms_Result := Common.Wait (Request, 0);
            begin
               raise Program_Error with C2.Outcome'Image (Common.Status (Unexpected));
            end;
         exception
         ...
         AMS.MEL.Close (Parent);
         C2.Close (Control);
         if Common.Report (Common.Wait (Request, 1_000)).Request_ID /= 19 then
            raise Program_Error;
         end if;
         Common.Close (Request);
      end;
      declare
         Parent  : AMS.MEL.Session := AMS.MEL.Open (Provider_Path, "comms-reject");
         Control : C2.Control_Channel := C2.Open (Parent, Config);
         Request : Common.Comms_Request := Common.Submit_Comms_Test (Control, 1, 2, 3);
         Result  : constant Common.Comms_Result := Common.Wait (Request, 1_000);
      begin
         if Common.Status (Result) /= C2.Rejected
           or else Common.Rejection_Code (Result) /= C2.Invalid_Parameters
           or else Common.Description (Result)'Length /= 613
         then
            raise Program_Error with "Ada Comms rejection mismatch";
         end if;
         Common.Close (Request);
         C2.Close (Control);
         AMS.MEL.Close (Parent);
   ...
   begin
      Test_Pre_Enable (Provider_Path);
      Test_Capability (Provider_Path);
      Test_Timeout_And_Rejection (Provider_Path);
      Ada.Text_IO.Put_Line ("PASS: Ada common IR C2 Channel contract");
   end Run;
