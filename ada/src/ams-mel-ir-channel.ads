...
package AMS.MEL.IR.Channel is
   ...
   type Channel_Type is (IRST_Track, IRST_Image, Command_And_Control,
      Scheduling, Health_And_Status, Instrumentation, Stacked_Image,
      Reserved_1, Reserved_2);
   type Pixel_Format is (Mono, RGB, Bayer);
   type Sensor_Type is (Unspecified, Gimbal_Horizontal, Gimbal_Vertical,
      Gimbal_Rotation, Step_Stare);
   type Metadata_Capability is (Bad_Pixel_List, Optical_Distortion_Map,
      LF_Status, Line_Of_Sight_Report, Line_Of_Sight_Quaternion,
      Line_Of_Sight_Euler, MFA_Status, MFA_Status_Detailed, BIT_Configuration,
      Command_Status, BIT_Status, Candidate_Object_Message, Task_Executing_Rep,
      Subsystem_Status_Resp, Execute_Task_Ack, Sched_Created_Rep,
      IRST_Track_Report, Channel_Comms_Test_Rep, Camera_Command_Resp,
      Camera_Protect_Cmd_Resp, Instrumentation_Report, Navigation_Report_Resp,
      Request_System_Track_Data, Update_Track_List_Response,
      LOS_3D_Kinematics_Type, Candidate_Object_Preproc_Message, Task_Events,
      Scan_Performance_Report, Reserved_3, Reserved_5, Reserved_9, Reserved_10);
   type Band_Type is (Invalid, Multiband, IR_Far, IR_Near, IR_Longwave,
      IR_Midwave, IR_Shortwave, Visible_White, Visible_Red, Visible_Green,
      Visible_Blue, UVA, UVB, UVC, UV_Vacuum);
   ...
   type Image_Band is record Band_Index : Interfaces.Unsigned_32; Bands : Band_Info_Vectors.Vector; end record;
   ...
   function Create_Capability
     (Channel_ID : UCI_ID; Height, Width, Bit_Depth, Row_Pitch, Buffer_Size,
      ...
      Nav_Frames : Coordinate_Vectors.Vector) return Channel_Capability;
   ...
   function Metadata_Capability_At (Value : Channel_Capability; Index : Positive) return Metadata_Capability;
   function Has_Metadata_Capability (Value : Channel_Capability; Item : Metadata_Capability) return Boolean;
   function Image_Band_Count (Value : Channel_Capability) return Natural;
   function Image_Band_Index_At (Value : Channel_Capability; Index : Positive) return Interfaces.Unsigned_32;
   function Image_Band_Info_Count (Value : Channel_Capability; Index : Positive) return Natural;
   function Image_Band_Info_At (Value : Channel_Capability; Band_Index, Info_Index : Positive) return Band_Info;
   function Nav_Frame_Count (Value : Channel_Capability) return Natural;
   function Nav_Frame_At (Value : Channel_Capability; Index : Positive) return Coordinate_System_Type;
private
   type Channel_Capability is record
      ID, Platform : UCI_ID; H, W, Depth, Pitch, Buffer, Image, Band_Count, Schedule : Interfaces.Unsigned_32;
      Pixel : Pixel_Format := Mono; Sensors : Sensor_Type_Vectors.Vector;
      Location : Component_Location; Channels : Channel_Type_Vectors.Vector;
      ODC, NUC : Boolean := False; Metadata : Metadata_Vectors.Vector;
      Bands : Image_Band_Vectors.Vector; Frames : Coordinate_Vectors.Vector;
   end record;
