...
package body AMS.MEL.IR.Health_Status is
   ...
   function Create_Config (Channel_ID : UCI_ID; Platform_ID : UCI_ID;
      Sensor_Location : Component_Location) return Health_Config is
     ((Channel_ID, Platform_ID, Sensor_Location));
   ...
   function Open (Parent : Session; Config : Health_Config) return Health_Channel is
      ...
      Raw : aliased C.IR_Health_Config_V1 :=
        ((C.Byte_Array_16 (Config.Channel.Value), String_View (Channel_Label)), 4,
         (C.Byte_Array_16 (Config.Platform.Value), String_View (Platform_Label)),
         (Interfaces.C.double (Config.Location.X), Interfaces.C.double (Config.Location.Y),
          Interfaces.C.double (Config.Location.Z), String_View (Key), String_View (System_Name)));
      D : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required : aliased C.Size_T := 0;
   begin
      if Parent.Handle = C.Null_Session then raise Provider_Error with "provider session is closed"; end if;
      return Result : Health_Channel do
         if C.IR_Health_Open (Parent.Handle, Raw'Access, Result.Handle'Access,
            D'Address, D'Length, Required'Access) /= C.Success then
            raise Provider_Error with Message (D);
         end if;
      end return;
   end Open;

   function Is_Open (Channel : Health_Channel) return Boolean is
   ...
   begin
      if C.IR_Health_Get_Capabilities (Channel.Handle, Owner'Access, D'Address,
         D'Length, Required'Access) /= C.Success then raise Provider_Error with Message (D); end if;
      if C.IR_Capability_View (Owner, Address'Access, D'Address, D'Length,
         Required'Access) /= C.Success then Release; raise Provider_Error with Message (D); end if;
      declare
         Raw : constant Cap_Access := To_Cap (Address);
         Sensors : V.Sensor_Type_Vectors.Vector; Channels : V.Channel_Type_Vectors.Vector;
         Metadata : V.Metadata_Vectors.Vector; Bands : V.Image_Band_Vectors.Vector;
      ...
      begin
         ...
         declare Result : constant V.Channel_Capability := V.Create_Capability
           (Copy (Raw.Channel_ID), Raw.Height, Raw.Width, Raw.Bit_Depth, Raw.Row_Pitch,
            Raw.Buffer_Size, Raw.Image_Size, Raw.Number_Of_Bands,
            V.Pixel_Format'Val (Raw.Pixel_Format), Sensors, Copy (Raw.Platform_ID),
            Create_Component_Location (Long_Float (Raw.Sensor_Location.Offset_X_M),
              Long_Float (Raw.Sensor_Location.Offset_Y_M), Long_Float (Raw.Sensor_Location.Offset_Z_M),
              Copy (Raw.Sensor_Location.Key), Copy (Raw.Sensor_Location.System_Name)),
            Channels, Raw.Task_Schedule_Depth, Raw.ODC_Available = 1,
            Raw.NUC_Available = 1, Metadata, Bands, Frames);
