...
package body AMS.MEL.IR is
   ...
   is (US.To_String (Location.Key_Value));
   function System_Name (Location : Component_Location) return String
   is (US.To_String (Location.System_Value));
   ...
   function Create_Component_Location
     (Offset_X_M  : Long_Float;
      ...
      Key         : String;
      System_Name : String) return Component_Location is
   begin
      Reject_NUL (Key, "Key");
   ...
   function Open_Image_Stream (Parent : Session; Config : Image_Config) return Image_Stream is
   ...
   is (Object.Handle /= C.Null_Stream);

   procedure Start (Object : in out Image_Stream) is
      Diagnostic : aliased Diagnostic_Array := [others => Interfaces.C.nul];
      Required   : aliased C.Size_T := 0;
      Status     : constant Interfaces.Integer_32 :=
        C.IR_Stream_Start (Object.Handle, Diagnostic'Address, Diagnostic'Length, Required'Access);
   begin
      if Status /= C.Success then
         raise Provider_Error with Message (Diagnostic);
      end if;
   ...
   function Empty_Raw_Frame return C.IR_Frame_V1
   is ((System_Time_NS      => 0,
   ...
   procedure Check_Receive_Status (Status : Interfaces.Integer_32; Diagnostic : Diagnostic_Array) is
   begin
      if Status = C.Timeout then
         raise Timeout_Error with "IR receive timed out";
      elsif Status = C.Stream_Stopped then
         raise Stream_Stopped with "IR stream stopped";
      elsif Status /= C.Success and then Status /= C.Buffer_Too_Small then
         raise Provider_Error with Message (Diagnostic);
      end if;
   end Check_Receive_Status;

   function Receive (Object : Image_Stream; Timeout_Milliseconds : Natural := 0) return Frame is
   ...
   begin
      ...
      return Result : Frame (Natural (Raw.Pixel_Required)) do
         ...
         Status :=
           C.IR_Stream_Receive
             (Object.Handle, 0, Raw'Access, Diagnostic'Address, Diagnostic'Length, Required'Access);
         Check_Receive_Status (Status, Diagnostic);
         if Status /= C.Success
           or else Raw.Pixel_Format /= 0
           or else Raw.Bits_Per_Pixel /= 8
           or else Raw.Number_Of_Bands /= 1
           or else Raw.Image_Type > 1
           or else Raw.Image_Flip > 3
         then
         ...
         Result.Subframe_ID := Raw.Subframe_ID;
         ...
         Result.Row_Offset := Raw.Row_Offset;
         Result.Column_Offset := Raw.Column_Offset;
   ...
   begin
      ...
      return
        (Counter (Raw.Frames_Received),
         Counter (Raw.Frames_Dropped_Queue_Full),
   ...
   procedure Stop (Object : in out Image_Stream) is
      ...
      Required   : aliased C.Size_T := 0;
      Status     : constant Interfaces.Integer_32 :=
        C.IR_Stream_Stop (Object.Handle, Diagnostic'Address, Diagnostic'Length, Required'Access);
   begin
      if Status /= C.Success then
         raise Provider_Error with Message (Diagnostic);
      end if;
   ...
   overriding
   procedure Finalize (Object : in out Image_Stream) is
