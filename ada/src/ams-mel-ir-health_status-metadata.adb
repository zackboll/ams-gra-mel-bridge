...
package body AMS.MEL.IR.Health_Status.Metadata is
   ...
   begin
      ...
      begin
         ...
         return
           (if Result'Length = 0 then "native IR Health metadata operation failed" else Result);
      end;
   end Message;
   ...
   begin
      if Raw.Active_BITS.Size > 0 then
         for I in 0 .. Natural (Raw.Active_BITS.Size) - 1 loop
            declare
               V : constant C.Active_BIT_V1 :=
                 To_Active
                   (Address_At
                      (Raw.Active_BITS.Data,
                       I,
                 ...
                 (Active,
                  S.Create_Active_BIT
                    (Copy (V.BIT_ID),
                     ...
                     Long_Float (V.Estimated_Percent_Complete)));
            end;
         end loop;
      end if;
      if Raw.Completed_BITS.Size > 0 then
         for I in 0 .. Natural (Raw.Completed_BITS.Size) - 1 loop
            declare
               V     : constant C.Completed_BIT_V1 :=
                 To_Completed
                   (Address_At
                      (Raw.Completed_BITS.Data,
                       I,
            ...
            begin
               ...
               if V.BIT_Items.Size > 0 then
                  for J in 0 .. Natural (V.BIT_Items.Size) - 1 loop
                     ...
                     begin
                        if X.Result > 4 then
                           raise Provider_Error with "invalid native BIT item result";
                        end if;
                        S.Append
                          (Items,
                           S.Create_Completed_BIT_Item
                             (Copy (X.BIT_Item_Name),
                              S.BIT_Result'Val (X.Result),
                              Copy (X.Fail_Reason)));
                     end;
                  end loop;
               end if;
               S.Append
                 (Completed,
                  S.Create_Completed_BIT
                    (Copy (V.BIT_ID),
                     Long_Long_Integer (V.Time_Tag_NS),
                     S.BIT_Result'Val (V.Result),
                     Copy (V.Fail_Reason),
                     Items));
            end;
         end loop;
      ...
      if Raw.Faults.Size > 0 then
         for I in 0 .. Natural (Raw.Faults.Size) - 1 loop
            declare
               V      : constant C.Fault_V1 :=
                 To_Fault
                   (Address_At
                      (Raw.Faults.Data, I, C.Fault_V1'Object_Size / System.Storage_Unit)).all;
               Data   : S.Fault_Data_List;
               Groups : S.Fault_Ambiguity_Group_List;
            begin
                  ...
                  for J in 0 .. Natural (V.Fault_Data.Size) - 1 loop
                     ...
                     begin
                        S.Append
                          (Data,
                           S.Create_Fault_Data
                             (Copy (X.Key), Copy (X.Value), Copy (X.Format), Copy (X.Units)));
                     end;
                  end loop;
               end if;
                  ...
                  for J in 0 .. Natural (V.Ambiguity_Groups.Size) - 1 loop
                     declare
                        X : constant C.Fault_Ambiguity_Group_V1 :=
                          To_Group
                            (Address_At
                               (V.Ambiguity_Groups.Data,
                                J,
                                C.Fault_Ambiguity_Group_V1'Object_Size / System.Storage_Unit)).all;
                     begin
                        S.Append
                          (Groups,
                           S.Create_Ambiguity_Group
                             (Copy_IDs (X.Diagnostic_Test_IDs), Copy_IDs (X.Component_IDs)));
                     end;
                  end loop;
               end if;
               S.Append
                 (Faults,
                  S.Create_Fault
                    (Copy (V.Fault_ID),
                     S.Fault_Severity'Val (V.Severity),
                     S.Fault_State'Val (V.State),
                     Long_Long_Integer (V.Detection_Time_NS),
                     Copy (V.Fault_Code),
                     Copy (V.Fault_Description),
                     Data,
                     Copy_IDs (V.Component_IDs),
                     Groups));
            end;
         end loop;
      end if;
   ...
   begin
      ...
      if Raw.Components.Size > 0 then
         for I in 0 .. Natural (Raw.Components.Size) - 1 loop
            declare
               V : constant C.MFA_Component_V1 :=
                 To_Component
                   (Address_At
                      (Raw.Components.Data,
                       I,
                       C.MFA_Component_V1'Object_Size / System.Storage_Unit)).all;
            ...
            begin
               ...
               S.Append
                 (Components,
                  S.Create_MFA_Component
                    (Copy (V.Component_ID),
                     S.Component_State'Val (V.State),
                     Long_Float (V.Temperature.Temperature_C),
                     S.Temperature_State'Val (V.Temperature.State),
                     S.Create_Foreign_Key
                       (Copy (V.Installation_Location_ID.Key),
                        Copy (V.Installation_Location_ID.System_Name)),
                     S.Create_Installation_Details
                       (Create_Component_Location
                          (Long_Float (L.Offset_X_M),
                           Long_Float (L.Offset_Y_M),
                           Long_Float (L.Offset_Z_M),
                           Copy (L.Key),
                           Copy (L.System_Name)),
      ...
      return
        S.Create_MFA_Status
          (S.MFA_State'Val (Raw.State),
           Copy (Raw.State_Description),
           Copy (Raw.Mode_Description),
           S.State_Transition_Status'Val (Raw.Transition_Status),
           S.Create_About
             (Copy (Raw.About_Data.Model),
              Copy (Raw.About_Data.Serial_Number),
              Copy (Raw.About_Data.Software_Version),
              Copy (Raw.About_Data.Bootloader_Software_Version),
              Copy (Raw.About_Data.Hardware_Version)),
           Components);
   end Copy_MFA;
   ...
   begin
      if Raw.Failure > 6 then
         raise Provider_Error with "invalid native subsystem failure";
      end if;
      Result :=
        (ID                  => Raw.Subsystem_ID,
         Importance          => Raw.Criticality,
         Sequence            => Raw.Status_Sequence_Number,
         Failure_Value       => Failure_Level'Val (Raw.Failure),
         Reported_Subsystems => Raw.Subsystem_Count,
         Subsystems          => <>,
         Reported_CSCI       => Raw.CSCI_Count,
         CSCIs               => <>);
      if Raw.Subsystems.Size > 0 then
         for I in 0 .. Natural (Raw.Subsystems.Size) - 1 loop
            declare
               V : constant C.IR_Subsystem_Dep_Info_V1 :=
                 To_Dependency
                   (Address_At
                      (Raw.Subsystems.Data,
                       I,
                       C.IR_Subsystem_Dep_Info_V1'Object_Size / System.Storage_Unit)).all;
            begin
               if V.Failure > 6 then
                  raise Provider_Error with "invalid native dependency failure";
               end if;
      ...
      if Raw.CSCI.Size > 0 then
         for I in 0 .. Natural (Raw.CSCI.Size) - 1 loop
            declare
               V : constant C.IR_Subsystem_CSCI_Info_V1 :=
                 To_CSCI
                   (Address_At
                      (Raw.CSCI.Data,
                       I,
                       C.IR_Subsystem_CSCI_Info_V1'Object_Size / System.Storage_Unit)).all;
            begin
               ...
               Result.CSCIs.Append
                 (Subsystem_CSCI'
                    (US.To_Unbounded_String (Copy (V.CSCI)),
                     CSCI_Mode'Val (V.Mode),
                     (V.Version.Source,
                      V.Version.Major_Revision,
                      V.Version.Minor_Revision,
                      V.Version.Engineering_Revision),
                     V.Criticality,
                     Failure_Level'Val (V.Failure),
                     V.BIT_Report,
   ...
   begin
      if Raw.Size > 0 then
         for I in 0 .. Natural (Raw.Size) - 1 loop
            declare
               V : constant C.Name_Value_Pair_V1 :=
                 To_Pair
                   (Address_At
                      (Raw.Data, I, C.Name_Value_Pair_V1'Object_Size / System.Storage_Unit)).all;
            begin
               S.Append (Result, S.Create_Name_Value_Pair (Copy (V.Name), Copy (V.Value)));
            end;
         end loop;
      end if;
      return Result;
   ...
   function Copy_Security (Raw : C.Security_Audit_Record_V1) return S.Security_Audit_Record is
   ...
   begin
      ...
      if Raw.Artifacts.Size > 0 then
         for I in 0 .. Natural (Raw.Artifacts.Size) - 1 loop
            declare
               V : constant C.Security_Artifact_V1 :=
                 To_Artifact
                   (Address_At
                      (Raw.Artifacts.Data,
                       I,
                       C.Security_Artifact_V1'Object_Size / System.Storage_Unit)).all;
            begin
               S.Append
                 (Artifacts,
                  S.Create_Security_Artifact (Copy (V.Component_ID), Copy (V.Associated_ID)));
            end;
         end loop;
      end if;
      return
        S.Create_Security_Audit_Record
          (Copy (Raw.Security_Event_ID),
           Long_Long_Integer (Raw.Event_Timestamp_NS),
           Copy (Raw.Subsystem_ID),
           Artifacts,
           S.Create_Security_Event
             (S.Security_Event_Kind'Val (Raw.Event.Kind),
              Natural (Raw.Event.Category),
              Copy (Raw.Event.Details),
              Copy (Raw.Event.Subsystem_ID),
              Copy (Raw.Event.Service_ID),
              Copy (Raw.Event.MDF_ID)),
           S.Security_Outcome'Val (Raw.Outcome),
   ...
   begin
      case Raw.Kind is
         when 1      =>
            return (MFA_Status_Event, Copy_MFA (Raw.MFA_Status));

         when 2      =>
            return (BIT_Status_Event, Copy_BIT (Raw.BIT_Status));

         when 3      =>
            return (Subsystem_Status_Event, Copy_Subsystem (Raw.Subsystem_Status));

         when 4      =>
            return (Discrete_Status_Event, Copy_Pairs (Raw.Discrete_Status));

         when 5      =>
            return (Security_Audit_Event, Copy_Security (Raw.Security_Audit));

         when 6      =>
            return (MFA_Status_Detailed_Event, Copy_Pairs (Raw.MFA_Status_Detailed));

         when others =>
            raise Provider_Error with "invalid native Health metadata kind";
      end case;
   end Copy_Event;

   function Open (Channel : Health_Channel; Queue_Capacity : Positive := 16) return Metadata_Stream
   is
      D        : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required : aliased C.Size_T := 0;
   begin
      if Channel.Handle = C.Null_Health then
         raise Provider_Error with "Health channel is closed";
      end if;
      return Result : Metadata_Stream do
         if C.IR_Health_Metadata_Open
              (Channel.Handle,
   ...
   is (Stream.Handle /= C.Null_Health_Metadata);
   ...
   begin
      ...
      if C.IR_Health_Event_View (Owner, Address'Access, D'Address, D'Length, Required'Access)
        /= C.Success
      then
         Release;
         raise Provider_Error with Message (D);
   ...
   exception
      when others =>
         if Owner /= C.Null_Health_Event then
            Release;
         end if;
         raise;
   end Receive;
   ...
   function Counters (Stream : Metadata_Stream) return Metadata_Counters is
      ...
      Required : aliased C.Size_T := 0;
   begin
      if C.IR_Health_Metadata_Get_Counters
   ...
   procedure Close (Stream : in out Metadata_Stream) is
      ...
      Required : aliased C.Size_T := 0;
   begin
      if C.IR_Health_Metadata_Close (Stream.Handle'Access, D'Address, D'Length, Required'Access)
        /= C.Success
      then
         raise Provider_Error with Message (D);
      end if;
   ...
   procedure Finalize (Stream : in out Metadata_Stream) is
      Ignored : Interfaces.Integer_32;
   begin
      Ignored := C.IR_Health_Metadata_Close (Stream.Handle'Access, System.Null_Address, 0, null);
   exception
      when others =>
   ...
   is (Event.Event_Kind);
   function MFA_Status_Value (Event : Metadata_Event) return S.MFA_Status
   is (Event.MFA);
   function BIT_Status_Value (Event : Metadata_Event) return S.BIT_Status
   is (Event.BIT);
   function Subsystem_Status_Value (Event : Metadata_Event) return Subsystem_Status
   is (Event.Subsystem);
   function Discrete_Status_Value (Event : Metadata_Event) return S.Discrete_Status
   is (Event.Discrete);
   function Security_Audit_Value (Event : Metadata_Event) return S.Security_Audit_Record
   is (Event.Security);
   function MFA_Status_Detailed_Value (Event : Metadata_Event) return S.MFA_Status_Detailed
   is (Event.Detailed);
