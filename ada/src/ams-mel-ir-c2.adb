...
package body AMS.MEL.IR.C2 is
   ...
   type Diagnostic_Array is array (Interfaces.C.size_t range <>) of aliased Interfaces.C.char
   with Convention => C;
   subtype Fixed_Diagnostic is Diagnostic_Array (0 .. Diagnostic_Capacity - 1);
   ...
   is ((Channel => Channel_ID, Platform => Platform_ID, Location => Sensor_Location));

   function Open (Parent : Session; Config : Control_Config) return Control_Channel is
      ...
      Raw            : aliased C.IR_C2_Config_V1 :=
        (Channel_Type    => 2,
         Channel_ID      =>
           (UUID              => C.Byte_Array_16 (Config.Channel.Value),
            Descriptive_Label => String_View (Channel_Label)),
         Platform_ID     =>
           (UUID              => C.Byte_Array_16 (Config.Platform.Value),
         ...
         Sensor_Location =>
           (Offset_X_M  => Interfaces.C.double (Config.Location.X),
            ...
            Key         => String_View (Key),
            System_Name => String_View (System_Name)));
   ...
   function Raw_Scan (Value : Scan_Parameters) return C.IR_Scan_Param_V1
   is ((Elevation_Defined_With_Range_And_Altitude =>
        ...
        Center_EL_Rad                             =>
          Interfaces.C.double (Value.Center_Elevation_Rad),
        Center_Frame_Ref_EL                       =>
          Coordinate_Frame_Reference'Enum_Rep (Value.Center_Frame_Reference_EL),
        Center_Frame_Ref_AZ                       =>
          Coordinate_Frame_Reference'Enum_Rep (Value.Center_Frame_Reference_AZ),
        Scan_Width_Rad                            => Interfaces.C.double (Value.Scan_Width_Rad),
        Scan_Height_Rad                           => Interfaces.C.double (Value.Scan_Height_Rad),
        Scan_Type                                 =>
          (Interfaces.Unsigned_32 (Value.Continuous_Scan),
        ...
        Scan_Rate_Rad_Per_Second                  =>
          Interfaces.C.double (Value.Scan_Rate_Rad_Per_Second),
        Preferred_Revisit_Interval_Seconds        =>
          Interfaces.C.double (Value.Preferred_Revisit_Interval_Seconds),
        Required_Revisit_Interval_Seconds         =>
          Interfaces.C.double (Value.Required_Revisit_Interval_Seconds),
        Max_Range_Of_Interest_M                   =>
          Interfaces.Unsigned_32 (Value.Max_Range_Of_Interest_M),
        Min_Range_Of_Interest_M                   =>
          Interfaces.Unsigned_32 (Value.Min_Range_Of_Interest_M),
        Elevation_Scan_Center_Altitude_M          =>
          Interfaces.Unsigned_32 (Value.Elevation_Scan_Center_Altitude_M),
        Elevation_Scan_Center_Range_M             =>
          Interfaces.Unsigned_32 (Value.Elevation_Scan_Center_Range_M),
        Degradation_Method                        =>
          Degradation_Method'Enum_Rep (Value.Degradation)));
   ...
   is
      Raw        : aliased C.IR_Mode_Command_V1 :=
        (Interfaces.Unsigned_32 (ID),
         MFA_State'Enum_Rep (State),
         MFA_Mode'Enum_Rep (Mode),
         Raw_Scan (Scan_Parameters));
      Diagnostic : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required   : aliased C.Size_T := 0;
   begin
   ...
   is (Request.Owner.Handle /= C.Null_Mode_Request);

   function Submit_BIT_No_Op (Channel : Control_Channel; ID : Command_ID := 0) return Return_Request
   is
      Diagnostic : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required   : aliased C.Size_T := 0;
   begin
   ...
   function Submit_BIT_IDs
     (Channel : Control_Channel; IDs : BIT_ID_Array; ID : Command_ID; Initiate : Boolean)
   ...
   function Submit_BIT_Initiate
     (Channel : Control_Channel; IDs : BIT_ID_Array; ID : Command_ID := 0) return Return_Request
   is (Submit_BIT_IDs (Channel, IDs, ID, True));

   function Submit_BIT_Cancel
     (Channel : Control_Channel; IDs : BIT_ID_Array; ID : Command_ID := 0) return Return_Request
   is (Submit_BIT_IDs (Channel, IDs, ID, False));

   function Submit_BIT_Clear_Faults
     (Channel : Control_Channel; Fault_Codes : Fault_Code_Vectors.Vector; ID : Command_ID := 0)
      return Return_Request
   is
      Views      : aliased C.String_View_Array (1 .. Positive (Fault_Codes.Length));
      Diagnostic : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required   : aliased C.Size_T := 0;
   begin
      if Channel.Handle = C.Null_C2 then
   ...
   function Submit_Config_Set
     (Channel        : Control_Channel;
      ID             : Command_ID := 0;
      System_Time_NS : System_Time_Nanoseconds := 0;
      Config         : String := "") return Return_Request
   is
      Raw        : aliased C.IR_Config_Set_Command_V1 :=
        (Interfaces.Unsigned_32 (ID), Interfaces.Integer_64 (System_Time_NS), String_View (Config));
      Diagnostic : aliased Fixed_Diagnostic := [others => Interfaces.C.nul];
      Required   : aliased C.Size_T := 0;
   begin
      if Channel.Handle = C.Null_C2 then
   ...
   function Wait (Request : Mode_Request; Timeout_Milliseconds : Natural) return Mode_Result is
      ...
      if Code = C.Timeout then
         ...
         return
           (Result_Status => Success,
            Result_Mode   => MFA_Mode'Val (Raw.Mode),
            Result_Code   => None,
         ...
         begin
            if Required > Diagnostic'Length then
               declare
                  ...
                  Retry_Raw      : aliased C.IR_Mode_Result_V1 := (Mode => 0, Error_Code => 0);
               ...
               begin
                  ...
                  then
                     raise Provider_Error
                       with "native IR C2 rejection changed during diagnostic retry";
                  end if;
   ...
   overriding
   procedure Finalize (Request : in out Request_Owner) is
      Ignored : Interfaces.Integer_32;
   begin
      Ignored := C.IR_Mode_Request_Close (Request.Handle'Access, System.Null_Address, 0, null);
   exception
      when others =>
         Request.Handle := C.Null_Mode_Request;
   end Finalize;

   overriding
   procedure Finalize (Request : in out Return_Request_Owner) is
      Ignored : Interfaces.Integer_32;
   begin
   ...
   overriding
   procedure Finalize (Channel : in out Control_Channel) is
      Ignored : Interfaces.Integer_32;
   begin
      Ignored := C.IR_C2_Close (Channel.Handle'Access, System.Null_Address, 0, null);
   exception
      when others =>
