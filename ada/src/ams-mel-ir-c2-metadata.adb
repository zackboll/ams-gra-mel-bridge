...
package body AMS.MEL.IR.C2.Metadata is
   ...
   function Open (Channel : Control_Channel; Queue_Capacity : Positive := 16) return Metadata_Stream is
      D : aliased Diagnostic := [others => Interfaces.C.nul]; R : aliased C.Size_T := 0;
   begin
      if Channel.Handle = C.Null_C2 then raise Provider_Error with "C2 channel is closed"; end if;
      return Result : Metadata_Stream do
         declare Code : constant Interfaces.Integer_32 := C.IR_C2_Metadata_Open
           (Channel.Handle, C.Size_T (Queue_Capacity), Result.Handle'Access, D'Address, D'Length, R'Access);
         begin if Code /= C.Success then raise Provider_Error with Message (D); end if; end;
      end return;
   end Open;
   ...
   begin
      ...
      if Raw.Kind = 1 then
         ...
         Result.Status := (C2.Command_ID (Raw.Command_Status.Command_ID), Command_State'Val (Raw.Command_Status.State), Cannot_Comply'Val (Raw.Command_Status.Reason_ID), US.To_Unbounded_String (Copy (Raw.Command_Status.Reason_Description)));
      elsif Raw.Kind = 2 then
         for I in 0 .. Natural (Raw.BIT_Configuration.BIT_Types.Size) - 1 loop
            declare B : constant C.BIT_Type_V1 := To_BIT_Type (Address_At (Raw.BIT_Configuration.BIT_Types.Data, I, C.BIT_Type_V1'Object_Size / System.Storage_Unit)).all; V : BIT_Type;
            begin
               if B.Accepted_Interface > 3 then raise Provider_Error with "invalid native BIT interface"; end if;
               V.ID := Copy (B.BIT_ID); V.Interface_Value := BIT_Control_Interface'Val (B.Accepted_Interface); V.Duration := Long_Long_Integer (B.Expected_Duration_NS);
               for J in 0 .. Natural (B.BIT_Item_Names.Size) - 1 loop V.Names.Append (US.To_Unbounded_String (Copy (To_String_View (Address_At (B.BIT_Item_Names.Data, J, C.String_View_V1'Object_Size / System.Storage_Unit)).all))); end loop;
         ...
         for I in 0 .. Natural (Raw.BIT_Status.Completed_BITS.Size) - 1 loop
            ...
            begin if B.Result > 4 then raise Provider_Error with "invalid native BIT result"; end if; V.ID := Copy (B.BIT_ID); V.Time := Long_Long_Integer (B.Time_Tag_NS); V.Value := BIT_Result'Val (B.Result); V.Why := US.To_Unbounded_String (Copy (B.Fail_Reason));
               for J in 0 .. Natural (B.BIT_Items.Size) - 1 loop declare X : constant C.Completed_BIT_Item_V1 := To_Item (Address_At (B.BIT_Items.Data, J, C.Completed_BIT_Item_V1'Object_Size / System.Storage_Unit)).all; begin if X.Result > 4 then raise Provider_Error with "invalid native item result"; end if; V.Items.Append (Completed_BIT_Item'(US.To_Unbounded_String (Copy (X.BIT_Item_Name)), US.To_Unbounded_String (Copy (X.Fail_Reason)), BIT_Result'Val (X.Result))); end; end loop; Result.Completed.Append (V); end;
         end loop;
         for I in 0 .. Natural (Raw.BIT_Status.Faults.Size) - 1 loop
            declare F : constant C.Fault_V1 := To_Fault (Address_At (Raw.BIT_Status.Faults.Data, I, C.Fault_V1'Object_Size / System.Storage_Unit)).all; V : Fault;
            begin if F.Severity > 4 or else F.State > 3 then raise Provider_Error with "invalid native fault enum"; end if; V.ID := Copy (F.Fault_ID); V.Severity_Value := Fault_Severity'Val (F.Severity); V.State_Value := Fault_State'Val (F.State); V.Time := Long_Long_Integer (F.Detection_Time_NS); V.Code := US.To_Unbounded_String (Copy (F.Fault_Code)); V.Description := US.To_Unbounded_String (Copy (F.Fault_Description)); V.Components := Copy_IDs (F.Component_IDs);
   ...
   function Receive (Stream : Metadata_Stream; Timeout_Milliseconds : Natural := 0) return Metadata_Event is
      Owner : aliased C.Metadata_Event_Handle := C.Null_Metadata_Event; Address : aliased System.Address := System.Null_Address; D : aliased Diagnostic := [others => Interfaces.C.nul]; R : aliased C.Size_T := 0; Code : Interfaces.Integer_32;
   begin
      Code := C.IR_C2_Metadata_Receive (Stream.Handle, Interfaces.Unsigned_32 (Timeout_Milliseconds), Owner'Access, D'Address, D'Length, R'Access);
      if Code = C.Timeout then raise Timeout_Error; elsif Code = C.Stream_Stopped then raise Stream_Stopped; elsif Code /= C.Success then raise Provider_Error with Message (D); end if;
      Code := C.IR_C2_Metadata_Event_View (Owner, Address'Access, D'Address, D'Length, R'Access);
      if Code /= C.Success then declare Ignored : constant Interfaces.Integer_32 := C.IR_C2_Metadata_Event_Close (Owner'Access, System.Null_Address, 0, null); begin raise Provider_Error with Message (D); end; end if;
      declare Result : constant Metadata_Event := Copy_Event (To_Event (Address).all); Ignored : constant Interfaces.Integer_32 := C.IR_C2_Metadata_Event_Close (Owner'Access, System.Null_Address, 0, null); begin return Result; end;
   exception when others => if Owner /= C.Null_Metadata_Event then declare Ignored : constant Interfaces.Integer_32 := C.IR_C2_Metadata_Event_Close (Owner'Access, System.Null_Address, 0, null); begin null; end; end if; raise;
   end Receive;

   function Kind (Event : Metadata_Event) return Metadata_Kind is (Event.Event_Kind);
   function Command (Event : Metadata_Event) return Command_Status is (Event.Status);
   function Command_ID (Value : Command_Status) return C2.Command_ID is (Value.ID);
   function State (Value : Command_Status) return Command_State is (Value.Status);
   function Reason (Value : Command_Status) return Cannot_Comply is (Value.Why);
   function Reason_Description (Value : Command_Status) return String is (US.To_String (Value.Description));
   function BIT_Type_Count (Event : Metadata_Event) return Natural is (Natural (Event.BIT_Types.Length)); function BIT_Type_At (Event : Metadata_Event; Index : Positive) return BIT_Type is (Event.BIT_Types (Index));
   function BIT_ID (Value : BIT_Type) return UCI_ID is (Value.ID); function Accepted_Interface (Value : BIT_Type) return BIT_Control_Interface is (Value.Interface_Value); function Expected_Duration_NS (Value : BIT_Type) return Long_Long_Integer is (Value.Duration); function BIT_Item_Name_Count (Value : BIT_Type) return Natural is (Natural (Value.Names.Length)); function BIT_Item_Name_At (Value : BIT_Type; Index : Positive) return String is (US.To_String (Value.Names (Index))); function Subsystem_Component_Count (Value : BIT_Type) return Natural is (Natural (Value.Components.Length)); function Subsystem_Component_At (Value : BIT_Type; Index : Positive) return UCI_ID is (Value.Components (Index));
   ...
   procedure Close (Stream : in out Metadata_Stream) is D : aliased Diagnostic := [others => Interfaces.C.nul]; R : aliased C.Size_T := 0; Code : constant Interfaces.Integer_32 := C.IR_C2_Metadata_Close (Stream.Handle'Access, D'Address, D'Length, R'Access); begin if Code /= C.Success then raise Provider_Error with Message (D); end if; end Close;
   overriding procedure Finalize (Stream : in out Metadata_Stream) is Ignored : Interfaces.Integer_32; begin Ignored := C.IR_C2_Metadata_Close (Stream.Handle'Access, System.Null_Address, 0, null); exception when others => Stream.Handle := C.Null_Metadata; end Finalize;
