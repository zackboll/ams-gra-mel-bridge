...
package AMS.MEL.IR.Image is
   ...
   type Directional is record X, Y, Z : Long_Float; end record;
   type Quaternion is record X, Y, Z, W : Long_Float; end record;
   type Nav_Error is record X, Y, Z, W : Long_Float; end record;
   ...
   type Orientation (Kind : Orientation_Kind := Euler_Orientation) is record
      case Kind is
         when Euler_Orientation => Euler_Value : AMS.MEL.Status.Euler;
         when Quaternion_Orientation => Quaternion_Value : Quaternion;
      end case;
   end record;
   ...
   function Receive (Object : AMS.MEL.IR.Image_Stream;
                     Timeout_Milliseconds : Natural := 0) return Full_Frame;
   ...
   function Sensor_Inertial_State_At (Value : Full_Frame; Index : Positive) return Sensor_Inertial_State;
...
private
   ...
   type Full_Frame is record
      ...
      Kind : Full_Image_Type; Flip : AMS.MEL.IR.Image_Flip;
      Flags : Flag_Vectors.Vector; D_Row, D_Column : Long_Float;
      Row, Column : Interfaces.Unsigned_32;
      Inertial : Inertial_Vectors.Vector; Nav : Nav_Vectors.Vector;
      Band : AMS.MEL.IR.Byte; Data : Pixel_Vectors.Vector;
   end record;
