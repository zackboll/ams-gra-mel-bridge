...
| IR C2 BIT no-op | Implemented in C and Ada | Command ID plus empty initiate/cancel/clear-fault lists; reusable async Return request |
...
| Real IR provider validation | Implemented and passed | Pinned Squall: C/Ada BIT no-op plus C/Ada/Rust/Python TaskSched and three 320x200 Mono8 frames; opt-in and excluded from ordinary CI |
...
| Safe Rust binding | Session + IR Mono8 + C2 Operate/TaskSched implemented | BIT is raw-sys-only; no safe ReturnRequest or additional C2/RF API |
