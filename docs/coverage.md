...
| Rust sys binding | Complete for the current project C ABI | Exactly 85 C functions; raw Instrumentation and complete Track declarations/constants synchronized, including the Track report/event layouts and the complete TrackDataUpdate/covariance/result layouts and update-request handle; no safe Instrumentation or Track API |
...
| Python binding | Session + IR Mono8 + C2 Operate/TaskSched + BIT no-op implemented | Complete current 85-function private ctypes binding; no public Image metadata/Navigation/Instrumentation/Track methods |
